...
CREATE TABLE IF NOT EXISTS tollgate_artifact_changes (
    ...
    FOREIGN KEY (decision_id) REFERENCES tollgate_decisions(decision_id)
);

CREATE INDEX IF NOT EXISTS idx_artifact_changes_artifact_time
    ON tollgate_artifact_changes(artifact_id, occurred_at);
CREATE INDEX IF NOT EXISTS idx_artifact_changes_project_time
    ON tollgate_artifact_changes(project_id, occurred_at);
